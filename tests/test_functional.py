...
class TestFunctional(unittest.TestCase):
    def test_functional(self):
        ...
        for test in os.listdir(FUNCTIONAL_TEST_DIR):
            ...
            if result:
                test_errors.append(
                    f"Functional test {test} failed with error: {result}!"
                )
    ...
    def compare_output_structure(self, test_name, test_dir):
        ...
        out_files_expected = [
            file for file in os.listdir(files_expected_path) if not file.startswith(".")
        ]
        out_tables_expected = [
            file
            for file in os.listdir(tables_expected_path)
            if not file.startswith(".")
        ]
        out_files_real = [
            file for file in os.listdir(files_real_path) if not file.startswith(".")
        ]
        out_tables_real = [
            file for file in os.listdir(tables_real_path) if not file.startswith(".")
        ]

        if set(out_files_real) != set(out_files_expected) or set(
            out_tables_real
        ) != set(out_tables_expected):
            return "Files do not match"

        error = ""

        match, mismatch_files, errors_files = filecmp.cmpfiles(
            files_real_path,
            files_expected_path,
            [os.path.basename(f) for f in out_files_expected],
        )
        match, mismatch_tables, errors_tables = filecmp.cmpfiles(
            tables_real_path,
            tables_expected_path,
